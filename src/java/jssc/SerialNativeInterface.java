...
public class SerialNativeInterface {
  ...
  private static final String libMinorSuffix = "2"; // since 0.9.0
  ...
  static {
    ...
    if (osName.equals("Linux")) {
      osName = "linux";
      SerialNativeInterface.osType = SerialNativeInterface.OS_LINUX;
    } else if (osName.startsWith("Win")) {
      osName = "windows";
      SerialNativeInterface.osType = SerialNativeInterface.OS_WINDOWS;
    }// since 0.9.0 ->
    else if (osName.equals("SunOS")) {
      osName = "solaris";
      SerialNativeInterface.osType = SerialNativeInterface.OS_SOLARIS;
    } else if (osName.equals("Mac OS X") || osName.equals("Darwin")) {// os.name "Darwin" since 2.6.0
      osName = "mac_os_x";
      SerialNativeInterface.osType = SerialNativeInterface.OS_MAC_OS_X;
    }// <- since 0.9.0
    ...
    if (architecture.equals("i386") || architecture.equals("i686")) {
      ...
      if (javaLibPath.toLowerCase().contains("gnueabihf") || javaLibPath.toLowerCase().contains("armhf")) {
        ...
        try {
          Process readelfProcess = Runtime.getRuntime().exec("readelf -l /proc/self/exe");
          BufferedReader reader = new BufferedReader(new InputStreamReader(readelfProcess.getInputStream()));
          while ((buffer = reader.readLine()) != null) {
            if (buffer.contains("Requesting program interpreter")) {
              lib = buffer.trim();
            }
          }
          reader.close();
          readelfProcess.waitFor();
        } catch (Exception ex) {
          // Do nothing
        }
        ...
        try {
          if (lib != null) {
            String l = lib;
            String s[] = l.split(Pattern.quote(": "));
            if (s.length == 2) {
              lib = s[1];
              lib = lib.substring(0, lib.length() - 1);
              Process readelfProcess = Runtime.getRuntime().exec("readelf -A " + lib);
              BufferedReader reader = new BufferedReader(new InputStreamReader(readelfProcess.getInputStream()));
              while ((buffer = reader.readLine()) != null) {
                if (buffer.toLowerCase().contains("Tag_ABI_VFP_args".toLowerCase())) {
                  floatStr = "hf";
                } else if (buffer.toLowerCase().contains("Tag_CPU_arch".toLowerCase())) {
                  l = buffer.trim();
                  s = l.split(Pattern.quote(": v"));
                  if (s.length == 2) {
                    armVersion = s[1];
                  }
                }
      ...
      architecture = "arm" + armVersion + floatStr;
    }

    libFolderPath = libRootFolder + fileSeparator + ".jssc" + fileSeparator + osName;
    libName = "jSSC-" + SerialNativeInterface.libVersion + "." + SerialNativeInterface.libMinorSuffix + "_" + architecture;
    ...
    if (SerialNativeInterface.isLibFolderExist(libFolderPath)) {
      if (SerialNativeInterface.isLibFileExist(libFolderPath + fileSeparator + libName)) {
        ...
        if (SerialNativeInterface.extractLib((libFolderPath + fileSeparator + libName), osName, libName)) {
          loadLib = true;
        }
      }
    } else {
    ...
    if (loadLib) {
      ...
      System.err.println("Warning! jSSC Native library not loaded! (looking for " + libFolderPath + fileSeparator + libName + ")");
    }
  }
  ...
  public static int getOsType() {
    return SerialNativeInterface.osType;
  }
  ...
  public static String getLibraryVersion() {
    return SerialNativeInterface.libVersion + "." + SerialNativeInterface.libMinorSuffix;
  }
  ...
  public static String getLibraryBaseVersion() {
    return SerialNativeInterface.libVersion;
  }
  ...
  public static String getLibraryMinorSuffix() {
    return SerialNativeInterface.libMinorSuffix;
  }
   ...
   *          handle of opened port
   * 
   * @return Method returns two-dimensional array containing event types and their values (<b>events[i][0] - event type</b>, <b>events[i][1] - event value</b>).
   */
