...
JNIEXPORT jbyteArray JNICALL Java_jssc_SerialNativeInterface_readBytes
  (JNIEnv *env, jobject object, jlong portHandle, jint byteCount){
    ...
    while (byteRemains > 0) {
        ...
        int retval = select(FD_SETSIZE, &read_fd_set, NULL, NULL, &timeout);
        if (retval < 0) {
            break;
        } else
        if (retval == 0) {
        ...
        if (FD_ISSET(portHandle, &read_fd_set)) {
    ...
    jbyte recevied = byteCount - byteRemains;
    jbyteArray returnArray = env->NewByteArray(recevied);
    if (recevied > 0) {
        env->SetByteArrayRegion(returnArray, 0, recevied, lpBuffer);
    }
    delete lpBuffer;
    return returnArray;
}
